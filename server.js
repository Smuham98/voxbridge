...
function sendWebPush(subscription, payloadObj) {
  // Safety check -- malformed subscription should never crash server
  if (!subscription || !subscription.endpoint || !subscription.keys) {
    return Promise.resolve({ error: 'invalid subscription' });
  }
...
app.get('/ice-config', async (req, res) => {
  ...
  // Try Metered.ca API for dedicated TURN credentials
  if (METERED_KEY) {
    try {
      ...
      console.log('Metered TURN fetch failed:', e.message);
    }
  }

  // Fallback: use multiple free TURN providers
  // These use the standard openrelay demo credentials
  iceServers = iceServers.concat([
    // OpenRelay - all ports and transports
...
// ── TRANSLATE ─────────────────────────────────────────
// -- TRANSCRIBE-FIX: clean accent-affected speech recognition --
app.post('/transcribe-fix', async (req, res) => {
  const { text, lang } = req.body;
  if (!text || !ANTHROPIC_API_KEY) return res.json({ fixed: text || '' });
  try {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        ...
        messages: [{ role: 'user', content: 'Fix this speech-to-text output from a person with a natural accent. The transcription may have misheard or dropped words. Return ONLY the corrected sentence, nothing else. If it looks fine already, return it unchanged.\nLanguage: ' + (lang||'unknown') + '\nTranscript: "' + text + '"' }]
      })
    });
    const d = await r.json();
    res.json({ fixed: d.content?.[0]?.text?.trim() || text });
  } catch(e) { res.json({ fixed: text }); }
});
...
io.on('connection', (socket) => {
  ...
  socket.on('call-user', ({ toUsername, fromUsername, fromDisplayName, fromLang, theirLang, offer, isVideo }) => {
    ...
    io.to(target.socketId).emit('incoming-call', { from: socket.id, fromUsername, fromDisplayName, fromLang, theirLang, offer, isVideo: !!isVideo });
    if (pushSubs[key]) {
  ...
  socket.on('answer-call', ({ to, answer }) => {
    ...
    // Send answerer's socket ID back so caller can route translation-ready events
    io.to(to).emit('call-answered', { answer, answererSocketId: socket.id });
  });
