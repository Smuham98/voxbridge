...
// -- ICE CONFIG for WebRTC (fresh credentials per call)
app.get('/ice-config', async (req, res) => {
  ...
  // If Metered API key is set, fetch real dedicated TURN credentials
  if (METERED_KEY) {
    try {
  ...
  // Fallback: multiple free TURN servers for redundancy
